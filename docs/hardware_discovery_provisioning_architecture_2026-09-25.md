...
| `internal/hardware/mr6c` | Modbus TCP, lease, сверка и применение эталона (запись по возрастанию адреса); чтение firmware через FC03 250–265 | Переиспользовать safety; добавить identity, проверку ожидаемого serial и безопасный порядок записи (раздел 8.3, шаг 4) |
...
Предлагаемый probe: FC04 модели → serial → firmware, затем FC03 сигнатуры. При `Illegal Data Address` на длинной модели пробовать старый диапазон из шести регистров. FC03 для информационных полей разрешать только в проверенном профиле. Текущий `readFirmware` уже читает 250–265 через FC03 при каждом подключении: этот путь тоже включается в проверяемый профиль прошивки или переводится на FC04. Сохранять сырой блок 266–271: раскладку MR6C нельзя автоматически приравнивать к раскладке MGE. Для MVP профиль legacy u32 допустим после сверки с наклейкой; будущая смена схемы serial оформляется явным alias, а не новым экземпляром.
...
- **Fast Modbus:** устройства участвуют в арбитраже по serial; можно получить разные serial даже при одинаковом адресе и адресовать команду конкретному serial. В эталонной утилите актуален FC `0x46`, legacy — `0x60`. [Эталон сканера](https://github.com/wirenboard/wb-modbus-ext-scanner/blob/de29548f0cdf3e2111d0e2a880879b129c4f112e/README.ru.md), [описание протокола](https://github.com/wirenboard/wb-modbus-ext-scanner/blob/de29548f0cdf3e2111d0e2a880879b129c4f112e/docs/protocol.ru.md).

MGE передаёт Fast Modbus в обоих режимах транспорта, но расширение `0x46` сам не реализует. В режиме Modbus TCP он пересылает кадры, удаляя преамбулу арбитража `0xFF`, в прозрачном — передаёт байты как есть. Собственная проба шлюза — FC `0x47` (`WB-FAST-MODBUS?`), только при `modbus=true` ([тест](https://github.com/wirenboard/wb-mge/blob/bf305f02f2688dccc2981832dc00e72b28149ee8/api_tests/26_test_fast_modbus_probe_e2e.py)). В wb-mqtt-serial Fast Modbus для TCP-портов с RTU включён всегда, для `modbus_tcp` — только с `connected_to_mge: true` ([`feature_port.cpp`](https://github.com/wirenboard/wb-mqtt-serial/blob/937284d82ccd5d616b5664b7febc566b3e7ca2de/src/port/feature_port.cpp)).

Для Go потребуется реализация расширения и тесты через реальный шлюз. Штатную C-утилиту с TTY-интерфейсом нельзя считать готовым Go API для Modbus TCP. MVP использует адрес с наклейки из манифеста. Запасной путь — Fast Modbus scan (раздел 11, этап 5): одиночный модуль он находит за секунды при любом адресе. Линейный перебор — последний вариант.
...
Для MR6C: сначала адрес из манифеста (с наклейки) или истории, затем Fast Modbus scan, когда он реализован; линейный поиск 1…247 — последний вариант. Для «Меркурия» на порту с одним счётчиком — `00 08 00` и заводской адрес по serial (3.3). На шине с несколькими счётчиками — `08 00` по ожидаемым адресам, затем ограниченный перебор. Разные семейства не пробовать вперемешку на работающем порту. На смешанной шине полный discovery и перенастройка допустимы только с отдельно проверенным профилем; в MVP такие шины требуют ручного описания.

При неизвестном UART использовать короткий упорядоченный список commissioning-профиля:
- MR6C: текущие параметры → 9600 8N2 → 115200 8N2;
...
При линейном переборе для пустых адресов использовать измеренный на стенде короткий таймаут пробы, а не таймаут цикла управления 300 мс. При 115 200 ответ MR6C занимает единицы миллисекунд плюс задержку шлюза; внутренний таймаут MGE учитывать. Найденный адрес подтверждать с обычным таймаутом.
...
При deadline 300 мс и одной попытке: MR6C — около 74 с, «Меркурий» — 72 с; три UART-профиля утраивают оценку. Это нижняя инженерная оценка по заданным параметрам: внутренний timeout MGE, reconnect и задержки настройки могут её увеличить. Для 100 портов при лимите 32 — около 5 минут плюс накладные расходы. Но полный перебор — запасной путь. При манифесте шлюз находится по MAC или hostname, MR6C — по адресу с наклейки, одиночный счётчик — за 1–2 транзакции через `00h`. Ввод поста тогда занимает секунды, а позже неизвестные адреса закрывает Fast Modbus.
...
4. Проверить exact profile MR6C, serial и firmware. Записать эталон `mr6c.SafetyConfig.Expected()` в таком порядке, чтобы ни одно промежуточное состояние не давало самопроизвольного переключения:
   ...
   Сейчас `applySafety` пишет по возрастанию адреса (5, 6, 8, 9…), поэтому для provisioning порядок задаёт профиль. Перечитать все поля, выходы и НЗ. Provisioning не выполняет пробное ON.
...
7. Перечитать saved и effective настройки MGE и `security_state`, снова идентифицировать периферию, проверить отсутствие конфликта и готовность всего комплекта. Только после этого публиковать runtime-конфигурацию.
...
|---|---|---|
| 0. Стенд и профили | Зафиксировать firmware MGE/MR6C, заводское состояние (режим портов, Wi-Fi, пароль, 504), маркировку и паспорта счётчиков, saved/effective API; подтвердить serial ↔ Ethernet MAC ↔ hostname, ответ «Меркурия» на `00h`, сброс watchdog кадрами к чужим адресам | Набор обезличенных ответов и таблица совместимости; проверка Unit 255 и 504 анализатором шины |
...
| 2. Offline-ввод | `evcomm-hw`: шлюз по MAC/hostname, MR6C по адресу с наклейки, счётчик через `00h`; защита MGE; эталон MR6C в безопасном порядке; генерация и проверка YAML. Сервис остановлен или посты выведены из работы | Известный комплект вводится без ручного редактирования YAML; у шлюза нет заводского пароля и точки доступа; все линии OFF |
| 3. Runtime и владение портами | StationManager, dial через владельца, lease gate для всех путей, commissioning supervisor | Пост добавляется без влияния на остальные; сканер не создаёт трафика к MR6C после потери lease; второй процесс не получает порт |
...
| 5. Provisioning в сервисе | План/diff, jobs и recovery, API; Fast Modbus как запасной поиск MR6C | Повторный apply не меняет совпавшие настройки; прерывание шага не даёт ложного ready; подтверждён поиск при совпадающих адресах |
...
| 7. Расширение | Смена адресов по serial, дополнительные meter profiles, интеграция wbmqtt | Каждый транспорт имеет одного владельца |
...
- расширить `mr6c-sim` identity-регистрами (сейчас FC04 не поддерживается), заводскими режимами входов и сменой адреса/скорости;
